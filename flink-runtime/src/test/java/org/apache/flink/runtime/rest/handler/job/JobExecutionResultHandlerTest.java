...
public class JobExecutionResultHandlerTest {
	...
	public void testCompletedResult() throws Exception {
		...
		when(mockRestfulGateway.getJobExecutionResult(any(JobID.class), any(Time.class)))
			.thenReturn(CompletableFuture.completedFuture(new JobExecutionResult.Builder()
				.jobId(TEST_JOB_ID)
	...
	@Test
	public void testPropagateJobExecutionResultNotFoundExceptionAsRestHandlerException() throws Exception {
		assertPropagateAsRestHandlerException(
			new CompletionException(new JobExecutionResultNotFoundException(new JobID())));
	}
